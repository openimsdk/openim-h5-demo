<template>
  <div class="page_container">
    ...
    <CheckedFooter v-if="showCheck" :all-checked-list="allCheckedList" :total="999" @remove="remove"
      @confirm="confirm" />
          ...
          <van-index-bar v-if="activeTab === 0 && renderFriendList.indexList.length > 0"
            ...
            <template v-for="(item, idx) in renderFriendList.indexList">
              ...
              <GenericListItem class="bg-white" v-for="(friend, index) in renderFriendList.dataList[idx]"
                ...
                :showCheck="showCheck" :disabled="disabledUserIDList.includes(friend.userID)" :checked="checkedUserList.findIndex((user) => user.userID === friend.userID) >
                  -1
    ...
    case ContactChooseEnum.ShareCard:
      allCheckedList.value.map(async (item) => {
        ...
        if (chooseType === ContactChooseEnum.ForwardMessage) {
          message = (
            await IMSDK.createForwardMessage(JSON.parse(history.state.extraData))
          ).data as MessageItem
        } else {
          message = (await IMSDK.createCardMessage(JSON.parse(history.state.extraData)))
            .data as MessageItem
        }
